...
procedure TfrmHerancaCadastro.VerificarEditsVazio;
var i : integer;
begin
  for i := 0 to ComponentCount - 1 do
  begin
    if (Components[i] is TDBEdit) and (TDBEdit(Components[i]).Field.IsNull)then
    begin
      MessageDlg('Preencha os campos Obrigatórios', mtInformation,[mbOk], 0);
      //ShowMessage('Preencha o campo ' + TDBEdit(Components[i]).Field.FieldName);
      Abort;
    end;
  end;
end;
